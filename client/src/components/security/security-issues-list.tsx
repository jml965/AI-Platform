import type { SecurityIssue } from "@/types/engine-result";

type SecurityIssuesListProps = {
  issues: SecurityIssue[];
};

function severityClasses(severity: SecurityIssue['severity']) {
  switch (severity) {
    case 'critical':
  ...
  if (!issues.length) {
    return (
      <div className="rounded-xl border border-white/10 bg-white/5 p-4 text-sm text-slate-300">
        لا توجد مشاكل أمنية مكتشفة في هذا الفحص.
      </div>
    );
  }
