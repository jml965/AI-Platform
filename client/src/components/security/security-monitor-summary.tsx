import type { SecurityMonitoringResult } from "@/types/engine-result";

type SecurityMonitorSummaryProps = {
  monitoring?: SecurityMonitoringResult | null;
};

function statusLabel(status: NonNullable<SecurityMonitoringResult["delta"]>["status"]) {
  switch (status) {
    case "first-run":
      return "First Run";
    case "improved":
...
function statusClasses(status: NonNullable<SecurityMonitoringResult["delta"]>["status"]) {
  switch (status) {
    case "first-run":
...
export default function SecurityMonitorSummary({ monitoring }: SecurityMonitorSummaryProps) {
    ...
    <div className="rounded-2xl border border-white/10 bg-white/5 p-5 space-y-4">
      ...
      {delta.status === "first-run" && (
        <div className="rounded-xl bg-blue-500/10 border border-blue-500/20 p-3 text-sm text-blue-200">
          هذه أول نتيجة محفوظة للمقارنة الأمنية.
        </div>
      )}
    </div>
  );
