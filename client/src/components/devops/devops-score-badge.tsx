type DevopsScoreBadgeProps = {
  score: number;
  grade: 'A' | 'B' | 'C' | 'D' | 'F';
};

function gradeClasses(grade: DevopsScoreBadgeProps['grade']) {
  switch (grade) {
    case 'A':
...
export default function DevopsScoreBadge({ score, grade }: DevopsScoreBadgeProps) {
  return (
    <div className={`inline-flex items-center gap-3 rounded-xl px-4 py-2 ${gradeClasses(grade)}`}>
      <span className="text-sm font-medium">DevOps Score</span>
