import type { DevopsFeedEvent } from "@/types/engine-result";

type DevopsFeedListProps = {
  feed?: DevopsFeedEvent[];
};

function eventClasses(type: DevopsFeedEvent["type"]) {
  switch (type) {
    case "devops-first-run":
      return "bg-blue-500/10 border border-blue-500/20 text-blue-200";
    case "devops-improved":
      return "bg-emerald-500/10 border border-emerald-500/20 text-emerald-200";
    case "devops-regressed":
      return "bg-red-500/10 border border-red-500/20 text-red-200";
    case "devops-unchanged":
    default:
      return "bg-slate-500/10 border border-slate-500/20 text-slate-200";
  }
}

export default function DevopsFeedList({ feed = [] }: DevopsFeedListProps) {
  if (!feed.length) {
    return (
      <div className="rounded-xl border border-white/10 bg-white/5 p-4 text-sm text-slate-300">
        لا توجد أحداث DevOps بعد.
      </div>
    );
  }
