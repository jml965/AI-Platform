...
- **Infrastructure Agents Panel:** A dedicated admin panel at `/infra` for platform owner to manage and develop the platform itself via AI agents. Features 7 infrastructure agents: System Monitor (Gemini, monitors health/performance), Bug Fixer (Claude, identifies and fixes bugs), Feature Builder (Claude, builds new features), UI Updater (Claude, updates design/layout), Database Manager (Claude, manages DB tables/data), Security Guard (Claude, scans vulnerabilities), Deployment Agent (Gemini, handles deployments/rollbacks). Each agent has its own streaming chat with context-aware system prompts including the full system blueprint (`system-blueprint.ts`). Agents are separated from customer-facing agents via `agentLayer` column (`"infra"` vs `"service"`) in `agent_configs` table. Backend routes: `GET /api/infra/agents`, `POST /api/infra/chat-stream`, `POST /api/infra/clear-session`.
