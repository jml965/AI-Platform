...
- **Live Preview:** React projects are previewed in-browser using Babel standalone + React/ReactDOM from CDN for transpilation. Static HTML/CSS/JS projects use direct srcDoc injection. The preview auto-refreshes when builds complete. Error display includes formatted error messages with line/column info and stack traces. Each file chunk is wrapped in an IIFE with an `__exports` registry to prevent icon name collisions (e.g., lucide-react `Home` icon vs `Home` component). Icon imports are converted to `var X = __icons["X"]` scoped locally within the IIFE, while component exports propagate via `__exports` between chunks. Navigation uses hash-based routing with `__matchRoute()` supporting dynamic route parameters (e.g., `/product/:id`). The IIFE import system has a `__skipImports` list preventing router/React mock names from being overwritten by `__exports`. `Link` passes through all props; `NavLink` supports `isActive` callback pattern; `RouterProvider` supports nested route matching.
