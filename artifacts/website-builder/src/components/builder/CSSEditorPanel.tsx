...
function ColorInput({ value, onChange, label }: { value: string; onChange: (v: string) => void; label: string }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="text-[11px] text-[#d4dae3] w-24 flex-shrink-0 truncate">{label}</span>
...
function PropertySection({ title, icon: Icon, children, defaultOpen = true }: {
  ...
  return (
    <div className="border-b border-[#1c2333]">
      <button
        onClick={() => setOpen(v => !v)}
        className="w-full flex items-center gap-2 px-3 py-2 text-[11px] font-semibold text-[#d4dae3] uppercase tracking-wider hover:bg-[#1c2333]/50 transition-colors"
      >
    ...
    <div className="w-[280px] flex flex-col bg-[#0d1117] border-s border-[#1c2333] flex-shrink-0 h-full">
      ...
      <div className="flex items-center gap-1 px-2 py-1.5 border-b border-[#1c2333] bg-[#161b22]">
        <button
          ...
          disabled={!canUndo}
          title={t.css_undo}
          className="p-1.5 rounded text-[#d4dae3] hover:text-[#e1e4e8] hover:bg-[#1c2333] disabled:opacity-30 disabled:hover:bg-transparent transition-colors"
        >
        ...
        {changeCount > 0 && (
          <>
            <button
              onClick={onClear}
              title={t.css_clear}
              className="p-1.5 rounded text-[#d4dae3] hover:text-red-400 hover:bg-red-500/10 transition-colors"
            >
      ...
      {!selectedElement ? (
        <div className="flex-1 flex items-center justify-center p-4">
          <div className="text-center">
            ...
            <p className="text-[13px] text-[#d4dae3]">{t.css_click_element}</p>
        ...
        <div className="flex-1 overflow-y-auto">
            ...
            {selectedElement.classList.length > 0 && (
              <div className="flex flex-wrap gap-1 mt-1">
                {selectedElement.classList.map((c) => (
                  <span key={c} className="text-[10px] px-1 py-0.5 rounded bg-[#1c2333] text-[#d4dae3] font-mono">.{c}</span>
                ))}
              </div>
            )}
          ...
          <PropertySection title={t.css_section_typography} icon={Type}>
            ...
            />
            <div className="flex items-center gap-2">
              <span className="text-[11px] text-[#d4dae3] w-24 flex-shrink-0">{t.css_text_align}</span>
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      styles.textAlign === align
                        ...
                        : "border-[#30363d] text-[#d4dae3] hover:text-[#e1e4e8] hover:bg-[#1c2333]"
                    )}
                  >
                    {label}
                  </button>
