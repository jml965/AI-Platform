          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className={cn(
                ...
                step.status === "done" ? "text-[#d4dae3] line-through" : "text-[#e1e4e8]"
              )}>
                {step.title}
              </p>
              {step.description && (
      ...
      {!isApproved && (onApprove || onModify) && (
        <div className="flex items-center gap-2 px-4 py-3 border-t border-[#1c2333]">
          ...
          {onModify && (
            <button
              onClick={onModify}
              className="flex items-center gap-1.5 px-3 py-1.5 text-[12px] font-medium text-[#d4dae3] border border-[#30363d] rounded-md hover:text-[#e1e4e8] hover:bg-[#1c2333] transition-colors"
            >
