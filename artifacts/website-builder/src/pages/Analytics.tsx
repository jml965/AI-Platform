  ...
  return (
    <div className="bg-[#161b22] border border-[#1c2333] rounded-xl p-4 hover:border-[#30363d] transition-colors">
      <div className="flex items-center gap-3 mb-2">
        ...
        <span className="text-sm text-[#d4dae3]">{label}</span>
      </div>
...
function EmptyState({ message }: { message: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-20 text-[#d4dae3]">
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    ...
    return (
      <div className="bg-[#161b22] border border-[#30363d] rounded-lg px-3 py-2 shadow-xl">
        <p className="text-xs text-[#d4dae3] mb-1">{label}</p>
    ...
    <div className="min-h-screen bg-[#0e1525] text-[#e1e4e8]">
      <header className="sticky top-0 z-10 bg-[#0d1117]/90 backdrop-blur-md border-b border-[#1c2333]">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-4">
          <Link
            href={`/project/${id}`}
            className="p-1.5 text-[#d4dae3] hover:text-[#e1e4e8] transition-colors rounded hover:bg-[#1c2333]"
          >
            <ArrowLeft className={cn("w-5 h-5", lang === "ar" && "rotate-180")} />
          </Link>
          <div className="flex-1">
            <h1 className="text-lg font-bold">{t.analytics_title}</h1>
            <p className="text-xs text-[#d4dae3]">{project?.name || t.loading}</p>
          </div>
          ...
          <>
            ...
            <div className="max-w-2xl mx-auto">
              <div className="bg-[#161b22] border border-[#1c2333] rounded-xl p-5">
                ...
                <p className="text-xs text-[#d4dae3] mb-3">{t.analytics_tracking_desc}</p>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={daily || []}>
                    ...
                    <XAxis
                      dataKey="date"
                      stroke="#484f58"
                      tick={{ fill: "#d4dae3", fontSize: 11 }}
                      tickFormatter={(v) => v.slice(5)}
                    />
                    <YAxis stroke="#484f58" tick={{ fill: "#d4dae3", fontSize: 11 }} />
              ...
              <div className="bg-[#161b22] border border-[#1c2333] rounded-xl p-5">
                ...
                {pages && pages.length > 0 ? (
                  <div className="space-y-2">
                    <div className="flex items-center text-xs text-[#d4dae3] px-3 py-1">
                    ...
                    {pages.map((page, i) => (
                      ...
                      >
                        ...
                        <span className="w-20 text-center text-sm text-[#d4dae3]">
                          {page.uniqueVisitors}
                        </span>
                      </div>
                    ))}
                  ...
                  <p className="text-sm text-[#d4dae3] text-center py-8">{t.analytics_no_data}</p>
                )}
              </div>
                    ...
                    {sources.map((source, i) => (
                      <div key={i} className="space-y-1">
                        <div className="flex items-center justify-between text-sm">
                          ...
                          <span className="text-[#d4dae3] flex-shrink-0 ms-2">
            ...
            <div className="bg-[#161b22] border border-[#1c2333] rounded-xl p-5">
              ...
              {devices ? (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  <div>
                    <h4 className="text-xs font-medium text-[#d4dae3] mb-3">{t.analytics_browsers}</h4>
                    <div className="h-48">
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                          <Pie
                    ...
                    <div className="space-y-1 mt-2">
                      {devices.browsers.map((b, i) => (
                        <div key={i} className="flex items-center justify-between text-xs">
                          ...
                          <span className="text-[#d4dae3]">{b.percentage}%</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div>
                    <h4 className="text-xs font-medium text-[#d4dae3] mb-3">
                    ...
                    <div className="h-48">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={devices.devices} layout="vertical">
                          <CartesianGrid strokeDasharray="3 3" stroke="#1c2333" />
                          <XAxis type="number" stroke="#484f58" tick={{ fill: "#d4dae3", fontSize: 11 }} />
                          <YAxis
                            type="category"
                            dataKey="name"
                            stroke="#484f58"
                    ...
                    <div className="space-y-1 mt-2">
                      {devices.os.map((o, i) => (
                        <div key={i} className="flex items-center justify-between text-xs">
                          ...
                          <span className="text-[#d4dae3]">{o.percentage}%</span>
                        </div>
                      ))}
                    </div>
                  </div>
