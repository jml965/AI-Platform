    ...
    return (
      <div className="min-h-screen bg-[#0d1117] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-[#d4dae3]">{t.loading}</p>
        </div>
      </div>
    );
  }
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium transition-all whitespace-nowrap ${
                tab === id
                  ...
                  : "bg-[#21262d] text-[#d4dae3] border border-[#30363d] hover:bg-[#30363d]"
              }`}
            >
  ...
  return (
    <div className="bg-[#161b22] border border-[#30363d] rounded-2xl p-5">
      <div className="flex items-center gap-3 mb-3">
        ...
        <span className="text-[#d4dae3] text-sm">{label}</span>
      </div>
      <p className="text-2xl font-bold text-white">{value}</p>
      {subtitle && <p className="text-xs text-[#d4dae3] mt-1">{subtitle}</p>}
    </div>
  );
}
...
function OverviewTab({ overview, daily, agents, t, lang }: {
  ...
  const pieData = agents.filter(a => a.cost > 0).map(a => ({
    ...
    value: a.cost,
    color: AGENT_LABELS[a.agentType]?.color || "#d4dae3",
  }));
        ...
        <div className="lg:col-span-2 bg-[#161b22] border border-[#30363d] rounded-2xl p-5">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <AreaChart data={daily}>
              ...
              <XAxis dataKey="date" stroke="#d4dae3" tick={{ fontSize: 11 }} tickFormatter={(v) => v.split("-").slice(1).join("/")} />
              <YAxis stroke="#d4dae3" tick={{ fontSize: 11 }} tickFormatter={(v) => "$" + v} />
              <Tooltip
            ...
            {pieData.map((d, i) => (
              <div key={i} className="flex items-center justify-between text-sm">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[#d4dae3]">{d.name}</span>
                </div>
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {agents.map((agent) => {
          const info = AGENT_LABELS[agent.agentType] || { ar: agent.agentType, en: agent.agentType, color: "#d4dae3" };
            ...
            <div key={agent.agentType} className="bg-[#161b22] border border-[#30363d] rounded-2xl p-5">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <span className="text-xs text-[#d4dae3]">{agent.agentType}</span>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div className="bg-[#0d1117] rounded-xl p-3">
                  <p className="text-xs text-[#d4dae3] mb-1">{t.runs}</p>
                  <p className="text-lg font-bold text-white">{agent.runs}</p>
                </div>
                <div className="bg-[#0d1117] rounded-xl p-3">
                  <p className="text-xs text-[#d4dae3] mb-1">{t.tokens}</p>
                  <p className="text-lg font-bold text-white">{formatNumber(agent.tokens)}</p>
                </div>
                <div className="bg-[#0d1117] rounded-xl p-3">
                  <p className="text-xs text-[#d4dae3] mb-1">{t.cost}</p>
                  <p className="text-lg font-bold text-[#f0883e]">{formatCost(agent.cost)}</p>
                </div>
                <div className="bg-[#0d1117] rounded-xl p-3">
                  <p className="text-xs text-[#d4dae3] mb-1">{t.avgDuration}</p>
              ...
              <div className="mt-4">
                <div className="flex items-center justify-between text-sm mb-2">
                  <span className="text-[#d4dae3]">{t.successRate}</span>
      ...
      <div className="bg-[#161b22] border border-[#30363d] rounded-2xl p-5">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={agents.map(a => ({
            ...
            <CartesianGrid strokeDasharray="3 3" stroke="#30363d" />
            <XAxis dataKey="name" stroke="#d4dae3" tick={{ fontSize: 11 }} />
            <YAxis stroke="#d4dae3" tick={{ fontSize: 11 }} tickFormatter={(v) => "$" + v} />
            <Tooltip
              contentStyle={{ background: "#161b22", border: "1px solid #30363d", borderRadius: 12, color: "#c9d1d9" }}
              formatter={(v: number) => formatCost(v)}
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-[#30363d]">
              <th className="px-5 py-4 text-start text-sm font-medium text-[#d4dae3]">{t.projectName}</th>
              <th className="px-5 py-4 text-start text-sm font-medium text-[#d4dae3]">{t.owner}</th>
              <th className="px-5 py-4 text-start text-sm font-medium text-[#d4dae3]">{t.status}</th>
              <th className="px-5 py-4 text-start text-sm font-medium text-[#d4dae3]">{t.tokens}</th>
              <th className="px-5 py-4 text-start text-sm font-medium text-[#d4dae3]">{t.cost}</th>
              <th className="px-5 py-4 text-start text-sm font-medium text-[#d4dae3]">{t.createdAt}</th>
            </tr>
          </thead>
          <tbody>
            {projects.map((p) => (
              <tr key={p.projectId} className="border-b border-[#30363d]/50 hover:bg-[#21262d]/50 transition-colors">
                ...
                <td className="px-5 py-4">
                  <div>
                    ...
                    <p className="text-[#d4dae3] text-xs">{p.userEmail}</p>
                  </div>
                </td>
                <td className="px-5 py-4">
                  <span
                    ...
                    style={{
                      color: statusColors[p.status] || "#d4dae3",
                      backgroundColor: (statusColors[p.status] || "#d4dae3") + "15",
                    }}
                  >
                    {p.status}
                  </span>
                ...
                <td className="px-5 py-4 text-sm text-[#d4dae3]">{new Date(p.createdAt).toLocaleDateString(lang === "ar" ? "ar-SA" : "en-US")}</td>
              </tr>
            ))}
          </tbody>
        </table>
    ...
    <div className="bg-[#161b22] border border-[#30363d] rounded-2xl overflow-hidden">
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead>
            <tr className="border-b border-[#30363d]">
              <th className="px-5 py-4 text-start text-sm font-medium text-[#d4dae3]">{t.displayName}</th>
              <th className="px-5 py-4 text-start text-sm font-medium text-[#d4dae3]">{t.email}</th>
              <th className="px-5 py-4 text-start text-sm font-medium text-[#d4dae3]">{t.role}</th>
              <th className="px-5 py-4 text-start text-sm font-medium text-[#d4dae3]">{t.projects}</th>
              <th className="px-5 py-4 text-start text-sm font-medium text-[#d4dae3]">{t.tokens}</th>
              <th className="px-5 py-4 text-start text-sm font-medium text-[#d4dae3]">{t.cost}</th>
              <th className="px-5 py-4 text-start text-sm font-medium text-[#d4dae3]">{t.balance}</th>
            </tr>
          </thead>
          <tbody>
            {users.map((u) => (
              <tr key={u.userId} className="border-b border-[#30363d]/50 hover:bg-[#21262d]/50 transition-colors">
                ...
                <td className="px-5 py-4 text-sm text-[#d4dae3]">{u.email}</td>
                <td className="px-5 py-4">
                  <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${
                    u.role === "admin" ? "bg-[#d2a8ff]/10 text-[#d2a8ff]" : "bg-[#d4dae3]/10 text-[#d4dae3]"
                  }`}>
