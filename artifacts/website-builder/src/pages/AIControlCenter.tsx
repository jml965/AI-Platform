...
function FieldHint({ text }: { text: string }) {
  return <p className="text-[9px] text-[#d4dae3]/70 mt-0.5 leading-relaxed">{text}</p>;
}
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-2 mt-0.5">
            ...
            <span className="text-[10px] text-[#d4dae3]">{provider.models.length} نموذج (models)</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div>
          <label className="text-[10px] text-[#d4dae3] mb-1 block"><BLabel ar="مفتاح API" en="API Key" /></label>
          ...
          <div className="flex gap-1.5 mt-1">
            <div className="relative flex-1">
              ...
              />
              <button onClick={() => setShowKey(!showKey)} className="absolute right-2 top-1/2 -translate-y-1/2 text-[#d4dae3] hover:text-white">
        ...
        <div>
          <label className="text-[10px] text-[#d4dae3] mb-1 block"><BLabel ar="رابط الحصول على المفتاح" en="Get API Key Link" /></label>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
        <div>
          <label className="text-[10px] text-[#d4dae3] mb-1 block"><BLabel ar="الأولوية" en="Priority" /></label>
        ...
        <div>
          <label className="text-[10px] text-[#d4dae3] mb-1 block"><BLabel ar="الميزانية الشهرية" en="Monthly Budget $" /></label>
        ...
        <div>
          <label className="text-[10px] text-[#d4dae3] mb-1 block"><BLabel ar="تنبيه عند %" en="Alert Threshold %" /></label>
        ...
        <div>
          <label className="text-[10px] text-[#d4dae3] mb-1 block"><BLabel ar="مزود احتياطي" en="Fallback Provider" /></label>
          ...
          {provider.models.map(m => (
            <div key={m.id} className="flex items-center justify-between bg-[#0d1117] border border-white/5 rounded-lg px-3 py-2">
              <div>
                ...
                <span className="text-[9px] text-[#d4dae3] ml-2">{m.id}</span>
              </div>
              <div className="flex items-center gap-3 text-[9px] text-[#d4dae3]">
          ...
          <div className="grid grid-cols-3 gap-2 mt-1.5">
            {[
              ...
              <div key={period.label} className="bg-[#0d1117] border border-white/5 rounded-lg p-2.5">
                <span className="text-[9px] text-[#d4dae3] block mb-1">{period.label}</span>
                <div className="text-[13px] font-bold text-green-400">${period.data.cost.toFixed(4)}</div>
                <div className="text-[9px] text-[#d4dae3] mt-0.5">
            ...
            {usage.recentLogs.slice(0, 20).map((log: any, i: number) => (
              <div key={i} className="flex items-center justify-between bg-[#0d1117] border border-white/5 rounded px-2.5 py-1.5 text-[10px]">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[#d4dae3]">{log.modelId}</span>
                  {log.agentKey && <span className="text-[#7c3aed]">{log.agentKey}</span>}
                </div>
                <div className="flex items-center gap-2 text-[#d4dae3]">
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-2 mt-0.5">
            <StatusBadge status={provider.keyStatus} />
            <span className="text-[10px] text-[#d4dae3]">{provider.models.length} {isImage ? "نموذج صور" : "نموذج فيديو"}</span>
      ...
      {provider.parentProvider && (
        <div className="bg-amber-500/5 border border-amber-500/15 rounded-xl p-3">
          ...
          <p className="text-[10px] text-[#d4dae3] leading-relaxed">
          ...
          <p className="text-[9px] text-[#d4dae3]/60 mt-1">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
        ...
        <div>
          <label className="text-[10px] text-[#d4dae3] mb-1 block"><BLabel ar="تنبيه عند %" en="Alert %" /></label>
          <FieldHint text="نسبة الميزانية للتنبيه" />
          <input type="number" min="50" max="100" value={provider.alertThreshold} onChange={e => onUpdate(provider.providerKey, { alertThreshold: parseInt(e.target.value) || 80 })} className="w-full bg-[#0d1117] border border-white/10 rounded-lg px-3 py-1.5 text-[11px] text-[#e2e8f0] mt-0.5" />
        </div>
        <div>
          <label className="text-[10px] text-[#d4dae3] mb-1 block"><BLabel ar="الحد الأقصى للملف" en="Max File Size" /></label>
          ...
          <div className="mt-0.5">
            ...
            />
            <div className="flex justify-between text-[9px] text-[#d4dae3] mt-0.5">
              <span>1 MB</span>
        ...
        <div className="grid gap-1.5 mt-1.5">
          {provider.models.map(m => (
            <div key={m.id} className="bg-[#0d1117] border border-white/5 rounded-lg px-3 py-2.5">
              ...
              <p className="text-[9px] text-[#d4dae3]/70 mt-1.5">{m.description}</p>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-[#0d1117] text-[#e2e8f0]" dir="rtl">
      <div className="flex items-center gap-3 px-4 py-3 border-b border-white/7 bg-[#161b22]">
        ...
        <div className="flex items-center gap-3 text-[10px] text-[#d4dae3]">
          ...
          <button
            ...
            className={`flex items-center gap-1 px-2 py-1 rounded-lg border transition-colors ${syncStatus?.syncInProgress ? "bg-blue-500/10 text-blue-400 border-blue-500/20 animate-pulse" : "bg-white/5 text-[#d4dae3] border-white/10 hover:text-white"}`}
          >
      ...
      {showSyncPanel && syncStatus && (
        <div className="px-4 py-3 border-b border-white/7 bg-[#0d1117]">
          <div className="bg-[#161b22] border border-white/7 rounded-xl p-4">
            ...
            <p className="text-[9px] text-[#d4dae3]/70 mb-3 leading-relaxed">
            ...
            <div className="grid grid-cols-4 gap-3 mb-3">
              <div className="bg-[#0d1117] border border-white/5 rounded-lg p-2.5">
                <div className="text-[9px] text-[#d4dae3] mb-0.5">آخر مزامنة (Last Sync)</div>
              ...
              <div className="bg-[#0d1117] border border-white/5 rounded-lg p-2.5">
                <div className="text-[9px] text-[#d4dae3] mb-0.5">المزامنة التالية (Next)</div>
                <div className="text-[11px] font-medium">{syncStatus.nextSyncAt ? new Date(syncStatus.nextSyncAt).toLocaleTimeString("ar") : "—"}</div>
              </div>
              <div className="bg-[#0d1117] border border-white/5 rounded-lg p-2.5">
                <div className="text-[9px] text-[#d4dae3] mb-0.5">الحالة (Status)</div>
              ...
              <div className="bg-[#0d1117] border border-white/5 rounded-lg p-2.5">
                <div className="text-[9px] text-[#d4dae3] mb-0.5">الفترة (Interval)</div>
                <select
            ...
            {syncStatus.results.length > 0 && (
              <div>
                <div className="text-[10px] text-[#d4dae3] mb-1.5">نتائج آخر فحص (Last Check Results)</div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-1.5">
                  {syncStatus.results.map(r => {
                    const statusColors: Record<string, string> = {
                      ...
                      skipped_no_key: "text-[#d4dae3] bg-white/5",
                    };
                    ...
                    return (
                      <div key={r.providerKey} className={`rounded-lg px-2 py-1.5 text-[9px] ${statusColors[r.status] || "bg-white/5 text-[#d4dae3]"}`}>
      ...
      <div className="grid grid-cols-4 gap-3 px-4 py-3 border-b border-white/7">
        <div className="bg-[#161b22] border border-white/7 rounded-xl p-2.5">
          <div className="text-[10px] text-[#d4dae3] mb-0.5">المزودون (Providers)</div>
          <div className="text-lg font-bold">{providers.length + mediaProviders.length}</div>
        </div>
        <div className="bg-[#161b22] border border-white/7 rounded-xl p-2.5">
          <div className="text-[10px] text-[#d4dae3] mb-0.5">مفاتيح نشطة (Active)</div>
          <div className="text-lg font-bold text-green-400">{activeCount}</div>
        </div>
        <div className="bg-[#161b22] border border-white/7 rounded-xl p-2.5">
          <div className="text-[10px] text-[#d4dae3] mb-0.5">نماذج النص (Text Models)</div>
          <div className="text-lg font-bold text-[#7c3aed]">{totalModels}</div>
        </div>
        <div className="bg-[#161b22] border border-white/7 rounded-xl p-2.5">
          <div className="text-[10px] text-[#d4dae3] mb-0.5">نماذج الوسائط (Media Models)</div>
      ...
      <div className="flex h-[calc(100vh-140px)]">
        <div className="w-64 border-l border-white/7 bg-[#0d1117] flex flex-col shrink-0">
          <div className="p-2">
            <div className="relative">
              <Search className="absolute right-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[#d4dae3]" />
              <input
                type="text"
                value={search}
            ...
            {([
              ...
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-1 py-2 text-[10px] font-medium border-b-2 transition-colors ${
                  activeSection === tab.key ? `${tab.color} border-current` : "text-[#d4dae3] border-transparent hover:text-white/60"
                }`}
              >
                {tab.icon}
                {tab.label}
            ...
            {filteredList.map((p: any) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[9px] text-[#d4dae3] truncate">{p.displayName}</div>
                </div>
                <span className="text-[9px] text-[#d4dae3] shrink-0">{p.models?.length || 0}</span>
              </button>
            ))}

            {filteredList.length === 0 && (
              <div className="text-center py-8 text-[#d4dae3]">
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {!selectedKey && (
            <div className="flex flex-col items-center justify-center h-full text-[#d4dae3]">
