  ...
  return [
    {
      ...
      content:
        [
          ...
          "Return only valid JSON.",
          "Generate a previewable website/app skeleton based on the user request.",
          ...
          "- output must be previewable",
          "- if Arabic content is appropriate, use Arabic and RTL",
          "- if intent is auction-platform, include auction-oriented sections/components",
          "- if intent is dashboard/admin-panel, include stats/tables/filters/layouts",
          "- if intent is marketplace, include listings/cards/search/filter patterns",
        ].filter(Boolean).join("\n"),
    },
    {
      role: "user" as const,
